...
func _is_host() -> bool:
	if not multiplayer.multiplayer_peer:
		return true   # single-player / offline
...
func _yearly_tick() -> void:
	for team in team_resources.keys():
		...
		if res["food"] >= pop:
			...
			var deficit := pop - res["food"]
			res["food"] = 0
			_starve(deficit, team)
		NetworkCommands.server_sync_resources(team)
...
func _spawn_child(team: int) -> void:
	...
	var child: Node3D = NetworkCommands.server_spawn_unit(
...
func register_artillery(art) -> void:
	var team := art.team if "team" in art else 0
	team_population.get(team, {})["population"] = \
		team_population.get(team, {}).get("population", 0) + 1
