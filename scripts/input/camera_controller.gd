...
## Input is LOCAL only. All game-state changes go through NetworkCommands RPCs
## so the host validates and executes them. Clients never mutate units directly.
...
func _confirm_placement() -> void:
	...
	# Compute placement position (same snap logic as ghost)
...
# ---------------------------------------------------------------------------
# Ghost helpers (unchanged from original)
# ---------------------------------------------------------------------------
